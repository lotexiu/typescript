import type { ParserGate, ParserNode } from '@ts/parser/node/model';
...
type TDelimitedSpec = {
	...
	invalidBy?: (
		code: number
	) => boolean | { before: (code: number) => boolean; after: (code: number) => boolean };
};
...
type TLangMatch = { ok: true; next: number; captures: TLangCapture[] } | { ok: false };
