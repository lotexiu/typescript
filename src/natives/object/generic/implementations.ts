...
function addPrefixToKeys<T extends TObject, Prefix extends string>(
	value: T,
	prefix: Prefix,
): TConcatStrIntoKeys<T, Prefix> {
	type PrefixedKeys = TKeyOf<T, { extract: string }, true>;
	const obj: any = {};
	(Object.getOwnPropertyNames(value) as PrefixedKeys[]).forEach(
		(key: PrefixedKeys): void => {
...
function removeNullFields<T extends object>(obj: T): Partial<T> {
	return (Object.entries(obj) as TEntriesReturn<T>[]).reduce(
		(acc: Partial<T>, [key, value]: TEntriesReturn<T>): Partial<T> => {
			if (!isNull(value)) {
...
function thisAsParameter<T extends TFunction>(
	fn: T,
...
function differenceBetweenObjects<T extends object>(
	objA: T,
	objB: T,
): Partial<T> {
